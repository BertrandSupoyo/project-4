...
async function generateRiwayatExcel(data, month, res, filename) {
  // Jika data berupa array substation (bukan {substation, siang, malam}), mapping dulu
  if (Array.isArray(data) && data.length > 0 && !data[0].substation) {
    data = data.map(sub => ({
      substation: sub,
  ...
  res.setHeader('Content-Disposition', `attachment; filename=${filename || `RiwayatPengukuran_${month}.xlsx`}`);
  await workbook.xlsx.write(res);
  res.end();
}

module.exports = { generateRiwayatExcel }; 